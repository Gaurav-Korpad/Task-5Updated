  ...
  return (
    <>
      ...
      <div className='text-center'>
        <Button type="submit" class="btn btn-success my-5" onClick={() => setPop(true)} > Click To Apply Leave</Button>
      </div>
      <Modal open={pop}
        ...
        <Box sx={{
          ...
          <form onSubmit={handleSubmit}>
            ...
            <FormControl sx={{ mb: 2 }}>
              ...
              <TextField className='border border-black' type='date' name='from' value={data.from} required onChange={handleChange} />
            </FormControl>
            <FormControl sx={{ ml: 2, mb: 2 }}>
              <FormLabel >To </FormLabel>
              <TextField className='border border-black' type='date' name='to' value={data.to} required onChange={handleChange} />
            </FormControl>

            <FormControl fullWidth>

              <FormLabel>Reason</FormLabel>
              <TextField className='border border-black' type='text' name='reason' multiline minRows={3} value={data.reason} required onChange={handleChange} />
            </FormControl>
          ...
          {staffData?.map((user) => {
            return <div className="col-3 mb-4 col-lg-4 col-md-6 col-sm-12">
              <div className='border p-2'>
                ...
                <h5> Date:- From:{user.from} - To:{user.to}</h5>
                <h4>Reason - </h4>
                <p>{user.reason}</p>
                <h4>Status - {user.status === "Pending.." &&
